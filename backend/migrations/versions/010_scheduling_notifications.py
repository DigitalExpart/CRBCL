...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "case_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("cases.id", ondelete="CASCADE"), nullable=True
        ),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.Column(
            "session_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("staffing_sessions.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
    ...
    op.add_column(
        "person_contacts", sa.Column("sms_consent", sa.Boolean(), nullable=False, server_default=sa.text("false"))
    )
    op.add_column(
        "person_contacts", sa.Column("email_consent", sa.Boolean(), nullable=False, server_default=sa.text("true"))
    )
